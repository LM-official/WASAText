...
eq  "11 member count"     3 "$(echo "$ABM" | grep -Eo '"[0-9a-f-]{36}"' | wc -l | tr -d ' ')"
hasnt "11 members contain no username" '"username":"' "$ABM"
hasnt "11 members contain no photo"     '"photo":"/photos/' "$ABM"
...
eq  "14 group holds three members" 3 "$(echo "$GCM" | grep -Eo '"[0-9a-f-]{36}"' | wc -l | tr -d ' ')"
hasnt "14 members contain no username" '"username":"' "$GCM"
hasnt "14 members contain no photo"     '"photo":"/photos/' "$GCM"
...
eq  "14 private holds two members" 2 "$(echo "$PC" | grep -o '"members":\[[^]]*\]' | grep -Eo '"[0-9a-f-]{36}"' | wc -l | tr -d ' ')"
