...
`doLogin`, `setMyUserName`, `setMyPhoto`, `getUsers`, `getMyConversations`, `getConversation`, `sendMessage`, `getPhoto`, `createPrivateChat`, `createGroup`, `setGroupName`, `setGroupPhoto`, `addToGroup`, `leaveGroup`.

Last full run: 2026-08-28, every section below re-checked against a freshly built binary: 372 assertions, all green — 305 from §0–15 and 67 from the §16 scenarios. The server log carried only the failures the tests asked for, and the invariants held afterwards — no chat without members, no private chat holding a name or a photo of its own, no group missing one, no membership pointing at a row that is gone, and every referenced photo id present on disk with nothing left over.
...
bash tests/run-tests.sh       # §0-15, 305 assertions (calls seed.sh itself)
bash tests/run-scenarios.sh   # §16 S1-S15, 67 assertions
```
...
| `dave` | belongs to nothing: the outsider of the `403` cases in §8–§11, §13 and §14 |
...
| `{"username":"carl","admin":true}` | `200`/`201` — unknown fields are ignored, see §17 |
...
| `Content-Type: application/json` | identical result — the handler never reads the header (§17) |
...
| own username with own token | `200`, and the caller is in the list — see §18 |
...
| `/photos/../../etc/passwd`, raw or percent-encoded | `404`, plain text — the extra segments match no route, so the router answers before the handler (§15) |
...
| `Content-Type: application/json` | identical result — the handler never reads the header (§17) |
| `{"name":"x","admin":true}` | `200` — unknown fields are ignored (§17) |
| `GET` / `POST` on the same path | `405 Method Not Allowed` (§15) |
...
| `GET` / `PATCH` / `DELETE` on the same path | `405 Method Not Allowed` + `Allow: OPTIONS, POST` (§15) |
...
| `GET` / `POST` / `PATCH` / `PUT` on the same path | `405 Method Not Allowed` + `Allow: DELETE, OPTIONS` (§15) |
...
| `POST` / `PATCH` / `DELETE` on the path | `405 Method Not Allowed` + `Allow: GET, OPTIONS` (§15) |
...
| `POST` / `PATCH` / `DELETE` on the path | `405 Method Not Allowed` (§15) |
| `GET /chats/{chatId}/` (trailing slash) | `404` (§15) |
| bad/absent token | §0 |
...
- One page of `ChatMessagesPageSize` (500), a `LIMIT` on the query and not a cut made afterwards, so a chat of any length is read in the same bounded time. Pagination is still missing (§18).
...
| `GET` on the same path | `405 Method Not Allowed` (§15) |
| bad/absent token | §0, and nothing is written to `./db/photos` |
...
## 15. Router level — the answers that are not JSON
...
## 16. Scenarios — a sequence, not a single call
...
## 17. Known mismatches with `doc/api.yaml` (implemented endpoints only)
Open:
