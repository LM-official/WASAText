...
bash tests/run-tests.sh       # §0-20, 532 assertions (calls seed.sh itself)
bash tests/run-scenarios.sh   # §21 S1-S20, 130 assertions
```
...
| `dave` | belongs to nothing: the outsider of the `403` cases in §8–§11 and §13–§19 |
...
| invalid/non-canonical `chatId` | `400 {"code":400,"message":"invalid chat id"}` |
| invalid/non-canonical `messageId` | `400 {"code":400,"message":"invalid message id"}` |
| `POST` / `DELETE` on the collection | `405 Method Not Allowed` (§20) |
| trailing slash | `404`, plain text (§20) |
| bad/absent token | §0, before either URL id is read |
