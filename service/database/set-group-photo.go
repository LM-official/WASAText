...
func (db *appdbimpl) SetGroupPhoto(userId schemas.UserId, groupId schemas.ChatId, newPhotoId schemas.PhotoId) (schemas.ChatBase, schemas.PhotoId, error) {
	...
	tx, err := db.c.Begin()
	if err != nil {
		return schemas.ChatBase{}, "", fmt.Errorf("cannot start the transaction: %w", err)
	}
	...
	if err := tx.Commit(); err != nil {
		return schemas.ChatBase{}, "", fmt.Errorf("cannot commit the transaction: %w", err)
	}
